---
name: fagemx-game-direction
description: |
  Game direction review from producer/creative director perspective.
  Challenges premises, evaluates scope vs capacity, market positioning, and strategic alignment.
  Use when scope feels wrong, pitching to stakeholders, or deciding continue/pivot/shelve.
  Do NOT use for from-scratch ideation (use fagemx-game-ideation) or GDD polish (use fagemx-game-review).
license: MIT
metadata:
  source: https://github.com/fagemx/gstack-game/tree/main/skills/game-direction
  upstream: fagemx/gstack-game
  tags: [game-design, direction, scope]
---

# Game Direction（精选摘录）

> 上游为 gstack-game 可调用技能（含 preamble / 遥测 / AskUserQuestion 样板）。本仓仅摘录可独立执行的方向评审流程；完整交互协议请回原仓。

## When to Use

- 「这个项目该不该做」「scope 是否过大」「市场定位对不对」
- 已有概念/GDD，需要从制作人/创意总监视角压力测试
- 决定 continue / pivot / shelve / kill

## Voice（摘要）

像做过上线、踩过延期坑的游戏开发者：先挑战 premise，再谈细节；用数字说话，禁止空夸「有潜力」。

## Routing（摘要）

- 从零概念 → `fagemx-game-ideation`
- 已有概念要挑战方向/scope → 本技能
- GDD 结构评审 → `fagemx-game-review`
- 手感/玩法实现 → `fagemx-feel-pass` / `fagemx-gameplay-implementation-review`
