---
name: fagemx-game-ideation
description: |
  Interactive game concept brainstorming. Structures raw ideas into Fantasy/Loop/Twist,
  challenges assumptions with forcing questions, and validates with Iceberg framework.
  Use when brainstorming from scratch or refining a vague concept into a testable GDD seed.
  Do NOT use for GDD review (use fagemx-game-review) or creative-direction challenge (use game-direction).
license: MIT
metadata:
  source: https://github.com/fagemx/gstack-game/tree/main/skills/game-ideation
  upstream: fagemx/gstack-game
  tags: [game-design, ideation]
---

# Game Ideation（精选摘录）

> 上游为 gstack-game 可调用技能（含 preamble / 遥测 / AskUserQuestion 格式）。本仓仅摘录可独立执行的设计流程；完整交互协议请回原仓。

## When to Use

- 「帮我想一个游戏概念」「从零头脑风暴」「Fantasy / Loop / Twist 怎么定」
- 有模糊点子，需要压成可验证的体验契约与验证计划

## Voice（摘要）

像做过上线、踩过延期坑的游戏开发者：挑战假设、反谄媚、优先可玩证据，而不是愿景幻灯片。

## Routing（摘要）

- 从零概念 → 本技能
- 已有 GDD 要评审 → `fagemx-game-review`
- 要挑战 scope / 市场定位 → `game-direction`（本轮观望，原仓可装）
- 要手感评审 → `fagemx-feel-pass`
