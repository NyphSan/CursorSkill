---
name: gamestudio
description: Use as the default full-game workflow when Codex should plan, build, debug, polish, QA, or finish a game project in one conversation when feasible, especially Godot, Unity, Phaser, WebGL, 2D prototypes, gameplay systems, maps, sprites, animation sheets, mobile UI/HUD, menus, shops, inventories, accessibility, localization, release readiness, asset integration, long-running phase handoff, or repeated bug investigation. Combines compact game-studio roles, minimal-solution discipline, mobile game UI/UX quality gates, 2D sprite/map asset routing, CODEX_HANDOFF.md continuity, and DEBUG_HANDOFF.md root-cause stop rules.
---
...
10. Before designing, changing, or reviewing mobile UI, load `references/ui-ux-audit.md`. New features must not increase permanent battle-screen information; consolidate, fold, or move secondary information when needed.
...
- Senior Game UI/UX Designer: mobile readability, HUD hierarchy, touch ergonomics, onboarding, retention risk, accessibility, responsive layout
...
## Mobile UI/UX Quality Gate

Load `references/ui-ux-audit.md` whenever the task designs, changes, or reviews UI, HUD, menus, shops, or inventories; targets phones; mentions small text, clutter, or hard-to-tap controls; adds information to a screen; or prepares a game release/update.

Treat readability as the first gate. Review a six-inch-phone experience, touch targets, visual hierarchy, information density, localization, safe areas, onboarding, retention, and store-review risk before visual polish. Do not solve overflow by shrinking text.

For Godot UI work, inspect relevant `Control`, `Container`, anchor, offset, size flag, minimum size, and `Theme` ownership before editing. Preserve gameplay, save, and billing behavior. Run the target-resolution and language checks defined in the reference, then output its required audit report.

Every new UI surface must pass this gate. If a new feature would add permanent battle HUD information, propose a fold, contextual reveal, detail page, or replacement for existing information before implementation.
...
- `references/ui-ux-audit.md`: mandatory mobile UI/HUD readability, touch, hierarchy, retention, Godot, and release audit
